   <div class="row">
    ...
    <table class='bordered striped'>
        <thead><tr><th data-field='id'>#</th><th data-field=''name''>Parametro</th><th data-field=''minimo''>Minimo</th><th data-field=''maximo''>Maximo</th><th data-field=''unidad''>Unidad</th><th data-field=''consecuencia''>Consecuencia</th><th data-field=''recomendacion''>Recomendacion</th><th data-field=''crop_id''>Cultivos</th><th data-field=''></th></tr></thead><tbody>
        <?php $count = 1;?>
        @foreach ($parameters as $key => $parameter)<tr>
           <td>{!! $count !!}</td>   
           <td data-field=''name''>{!! $parameter->name !!}</th>
           <td data-field=''minimo''>{!! $parameter->minimo !!}</th>
           <td data-field=''maximo''>{!! $parameter->maximo !!}</th>
           <td data-field=''unidad''>{!! $parameter->unidad !!}</th>
           <td data-field=''consecuencia''>{!! $parameter->consecuencia !!}</th>
           <td data-field=''recomendacion''>{!! $parameter->recomendacion !!}</th>
           <td data-field=''crop_id''>{!! $parameter->crops()->first()->name !!}</th>
        <td>
